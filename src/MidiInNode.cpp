...
MidiInNode::MidiInNode(std::string_view                 input_name,
    ...
    : Node(scale_provider, midi_input_node.get(), false, true), m_input_name(input_name),
...
void MidiInNode::render_inspector_internal()
{
...
std::string MidiInNode::get_name()
{
    check_midi_node_connected();
    return m_port_name_display->get_port_name(m_input_name);
}
